...
Release 15.35.2 — September 19, 2026
### 🐞 Bug Fixes
- avoid unsupported SQL aggregate field syntax (returns)
_Total: 1 changes_
...
Release 15.35.1 — September 19, 2026
### 🐞 Bug Fixes
- allow split cash+card payments to cover invoice total 
- export offline item price record types (docs)
- bulk load submitted invoice metadata (invoice-management)
- clarify start and end date filters (invoice-management)
- improve filters and lazy tab loading (invoice-management)
- show only actionable repair candidates (invoice-management)
- filter selected invoices before query limit (invoice-repair)
- support exact split follow-up allocations (invoice-repair)
_Total: 8 changes_
...
Release 15.35.0 — September 5, 2026
### ✨ Features
- brighten actions and resume locked payment after PIN (pos)
- add colorful cashier actions and readable cart headers (ui)
- polish cart and payment cashier workflows (ui)
- polish POS with accessible clinical-retail design system (ui)
_Total: 4 changes_

### 🐞 Bug Fixes
- align offline UOM pricing with online item prices and rules 
- fit all selected invoice columns in desktop pane (pos)
- keep active terminal session alive during invoice submission (pos)
- restore offline invoice dialog close icon (pos)
- contain cart values within table cells (ui)
- enforce accessible pay action color across themes (ui)
- refine light actions and responsive cart columns (ui)
_Total: 7 changes_
...
Release 15.34.0 — August 27, 2026
### ✨ Features
- complete multi-currency payment settlement and tender reporting 
- add generic per-payment multi-currency support (pos)
- add per-entry multi-currency support to payment entries (pos)
- show change in shortcut payment confirmation (pos)
- support multi-batch allocation and invoice row splitting (pos)
_Total: 5 changes_

### 🐞 Bug Fixes
- enforce subcontracting guard through invoice controllers 
- enhance customer search across identifiers 
- guard empty subcontracting lookup on POS returns 
- manual selection of batch 
- preserve exact tender precision during invoice submission 
- settle auto remainder with exact multi-currency base amount 
- preserve large pagination offsets (offline)
- separate tender currency from payment account currency (payments)
- auto-fill remaining tender and reset invoice currency (pos)
- keep multi-currency payment rows stable and honor rounded totals (pos)
- preserve exact multi-currency payment totals after v16 save (pos)
- preserve overpayment tender from shortcut dialog (pos)
- restore multi-currency tender totals before invoice submit (pos)
- resume pending invoice after terminal unlock (pos)
- retry v16 deadlocks and preserve exact multi-currency GL amounts (pos)
- support missing change GL setting during invoice submit (pos)
- apply subcontracting guard through v16 controller extension (returns)
- base the cash-refund cap on real payments, not paid_amount (returns)
- use the backend refundable cap instead of recomputing it (returns)
_Total: 19 changes_
...
Release 15.33.0 — July 23, 2026
### ✨ Features
- finalize RetailMind POS counter grid 
- restore POS Awesome branding and add profile options 
- speed up secure terminal unlock flow 
- add Counter Grid fast tender shortcuts (pos)
- add Counter Grid operational health strip (pos)
- add keyboard navigation for cashier workflow (pos)
- add quick manual batch and serial selection (pos)
- add safe Counter Grid exact entry (pos)
- add supervisor sale-floor policy (pos)
- show barcode and warehouse stock on variant cards (pos)
_Total: 10 changes_

### 🐞 Bug Fixes
- apply pricing rules while preserving manual POS rates when ignore pricing rule enabled 
- apply transaction pricing rules after offline refresh 
- batch no visibility for selection 
- harden POS item editing and sale controls 
- improve serial and batch chip UI 
- keep cash shortcuts independent of M-Pesa readiness 
- keep customer search responsive and prevent duplicates 
- make cashier unlock instant after reload 
- make customer mobile search indexed and cache resilient 
- normalize quantity pricing rule reconciliation 
- preserve dialog tab navigation 
- prevent POS credential autofill 
- reapply pricing rules after quantity changes 
- respect cart qty focus setting 
- use Frappe v15-compatible terminal state cache API 
- pass precision into batch quantity context (items-selector)
- preserve reserved batch stock (items-selector)
- preserve catalog during Counter Grid reload (items)
- guard queries against uninitialized v16 grids (pos-profile)
- allow arrow navigation from cashier inputs (pos)
- enable invoice optional column toggles (pos)
- harden below-cost sale guard (pos)
- harden startup network and catalog lifecycle (pos)
- improve variant card size and stock details layout (pos)
- recover from slow startup requests (pos)
- recover item catalog after cache failures (pos)
- remove residual startup hydration stalls (pos)
- satisfy Counter Grid health types (pos)
- show item catalog sync timestamp (pos)
- streamline keyboard item entry flow (pos)
- unblock catalog cold start (pos)
- wait for worker readiness before batch timeout (pos)
- align Counter Grid with negative-stock policy (stock)
- make new POS workflows dark-mode aware (ui)
_Total: 34 changes_
...
Release 15.32.0 — July 8, 2026
### ✨ Features
- add POS Profile raw ESC/POS printing for documents (printing)
_Total: 1 changes_

### 🐞 Bug Fixes
- Add partial receipt and bill modal for purchase management 
- Add purchase management for receipts bills and payments 
- added confirmation message before print fall back with reason. 
- align purchase payment allocation and raw print i18n 
- allign print related settings in print tab in pos profile 
- allow zero quick cash shortcut for credit sale 
- legacy-matched free lines 
- location ko customer credit fields 
- loyalty points issues and tests issues 
- normalize purchase order dates before submit 
- persist QZ Tray printer default on POS Profile 
- pos_profile can be resolved from either doc or source_doc 
- preserve inclusive tax contract during POS order invoicing 
- preserve non-inclusive POS VAT on draft submit 
- print callers QZ route for raw printing 
- Refresh POS Profile printing field layout on migrate 
- relocate custom fields after payment table 
- restrict fallback printing for debugging qz tray printing 
- derive loyalty points in company currency (pos)
- normalize loyalty redemption before invoice submit (pos)
- prevent duplicate pricing rule reward items on payment (pos)
- respect pricing rule minimum amount for free items (pos)
- match original row by reference for partial multi-batch returns (returns)
- Buttons Colors and allignment (style)
- improve purchase order draft actions and preview (style)
_Total: 25 changes_
...
### 🐞 Bug Fixes
- preserve order mode through POS payment flow 
- unify POS document type resolution 
_Total: 2 changes_
...
Release 15.29.3 — May 21, 2026
### 🐞 Bug Fixes
- Add loyalty redemption summary to POS closing shift 
- code review feedback and remove redundant get_doc in customer balance 
- create exchange gain/loss deduction row in multi-currency payments 
- customer balance display with company scoping, overflow protection, and POS Session DocType guard 
- customer cache readiness after POS reloads 
- harden POS loyalty, customer sync, and payment edge cases 
- loyalty customer sync with server-driven cache resync 
- loyalty redemption account handling on invoice submit 
- loyalty redemption visibility at checkout 
- loyalty redemption visibility in payments 
- manual base paid override with enterprise hardening + exchange gain/loss deduction rows 
- N+1 query problem in customer and use flt in overview 
- normalize blank valid_from / valid_upto values with _optional_date() 
- paginated customer sync persistence after POS refresh 
- POS customer selection before payment loyalty redemption 
- preserve customer cache and payment base values 
- resolve all PR review comments for customer balance display 
- syncs customer info immediately to calculate available_points_amount 
_Total: 18 changes_

...
export function usePaymentSubmission(options: PaymentSubmissionOptions) {
	...
	const submitInvoice = async (
		...
		try {
			...
			// === PINE LABS INTERCEPTION ===
			const pineLabsPayment = (submissionDoc.payments || []).find((p: any) => 
				p.mode_of_payment && String(p.mode_of_payment).toLowerCase().includes("pinelabs")
			);
			
			if (pineLabsPayment && type === "Invoice") {
				// Ensure it's treated as a draft for this step
				const draftDoc = JSON.parse(JSON.stringify(submissionDoc));
				draftDoc.docstatus = 0;
				
				// 1. Save as Draft
				const draftResponse = await frappe.call({
					method: "posawesome.posawesome.api.invoices.update_invoice",
					args: {
						data: JSON.stringify(draftDoc)
					},
					freeze: true,
					freeze_message: __("Saving Draft for Terminal...")
				});
				
				const draftName = draftResponse?.message?.name;
				if (!draftName) {
					throw new Error(__("Failed to create draft invoice for terminal payment."));
				}
				
				// Update the submission document with the draft name so submitInvoice updates it
				submissionDoc.name = draftName;
				if (doc) doc.name = draftName;
				
				// 2. Trigger Pine Labs
				await new Promise((resolve, reject) => {
					const refDoctype = profile?.create_pos_invoice_instead_of_sales_invoice ? "POS Invoice" : "Sales Invoice";
					
					// Function to poll the terminal status
					const pollStatus = () => {
						frappe.call({
							method: "pinelabs_fin.api.terminal.check_terminal_status",
							args: {
								reference_doctype: refDoctype,
								reference_name: draftName
							},
							callback: function(res: any) {
								...
								if (status === "SUCCESS") {
									...
									reject(new Error(__("Terminal Payment Failed.")));
								} else if (status === "CANCELLED") {
									reject(new Error(__("Terminal Payment Cancelled.")));
								} else {
							...
							error: function() {
								reject(new Error(__("Network error while checking terminal status.")));
							}
						});
					};
					...
					frappe.call({
						...
						args: {
							reference_doctype: refDoctype,
							reference_name: draftName,
							amount: pineLabsPayment.amount,
							mode_of_payment: pineLabsPayment.mode_of_payment,
							create_payment_entry: 0 // We handle accounting in POSAwesome
						},
						freeze: true,
						freeze_message: __("Waiting for Terminal Tap/Insert..."),
						callback: function(res: any) {
							if (res.message?.status === "SUCCESS") {
								resolve(true);
							} else if (res.message?.status === "FAILED") {
								reject(new Error(__("Failed to initiate terminal payment.")));
							} else {
								// Transaction is PENDING on the device, start polling
								pollStatus();
							}
						},
						error: function() {
							reject(new Error(__("Failed to reach terminal integration.")));
						}
					});
				});
			}
			// === END PINE LABS INTERCEPTION ===
